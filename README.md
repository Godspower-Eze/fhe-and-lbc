...
2. Encryption:
    ...
    3. $u = A^Tr \pmod{q}$ where $A^T$ is transpose of $A$
    4. $v = b^Tr + m.\lfloor q/2 \rfloor \pmod{q}$ where $b^T$ is the transpose of $b$ and $\lfloor q/2 \rfloor$ is the floor of $q/2$
3. Decryption:
    1. Compute $v - \langle u, s \rangle$ where $\langle u, s \rangle$ is the inner product of vectors $u$ and $s$. The result should be $e^Tr + m\lfloor q/2 \rfloor$
