...
pub fn encrypt(public_key: &(Vec<Vec<i128>>, Vec<i128>), r: &Vec<i128>, message: u8, q: i128) -> (Vec<i128>, i128) {
    ...
    println!("{:?}, {:?}, {:?}, {:?}, {}", a_t, public_key.1, r, u, v);
    (u, v)
}
...
fn decrypt(ciphertext: &(Vec<i128>, i128), secret_key: &Vec<i128>, q: i128) -> i128 {
    ...
    println!("{}, {:?}, {}, {}", u_s, secret_key, result, encoded_message);
    let mid_point = q / 2;
...
#[cfg(test)]
mod test {
    use crate::{pub_lwe::{center_mod, decrypt, encrypt, generate_public_key}, utils::{generate_random_bit_vector, generate_random_matrix, generate_random_vector, sample_discrete_gaussian_vector}};
    ...
    fn test_encrypt_and_decrypt() {
        ...
        let message = 0;
        let ciphertext = encrypt(&public_key, &r, message, q as i128);
